...
POSTGRES_PASSWORD=aicourt_sec_$(openssl rand -hex 16)
POSTGRES_DB=aicourt
...
ALLOWED_ORIGINS=http://$(curl -s ifconfig.me 2>/dev/null || echo 'YOUR_SERVER_IP')
...
main() {
    ...
    OFFLINE_DIR=""
    for candidate in "$SCRIPT_DIR" "$SCRIPT_DIR/offline-images" "$SCRIPT_DIR/../offline-images"; do
        if [ -d "$candidate" ] && find "$candidate" -maxdepth 1 -name "*.tar" | head -1 | grep -q .; then
    ...
    if [ -n "$OFFLINE_DIR" ]; then
        TAR_COUNT=$(find "$OFFLINE_DIR" -maxdepth 1 -name "*.tar" | wc -l)
        log_info "检测到离线镜像包: $OFFLINE_DIR ($TAR_COUNT 个文件)"
        echo ""
        echo -e "  ${GREEN}1)${NC} 在线部署 (从阿里云镜像源拉取)"
        echo -e "  ${GREEN}2)${NC} 离线部署 (使用本地 tar 镜像包，推荐)"
        echo ""
        read -p "请选择部署方式 [1-2, 默认2]: " DEPLOY_MODE
        DEPLOY_MODE=${DEPLOY_MODE:-2}

        if [ "$DEPLOY_MODE" = "2" ]; then
            log_info "使用离线部署模式"
            bash "$SCRIPT_DIR/install-offline.sh"
            exit $?
        fi
    fi
    ...
    configure_env "$INSTALL_DIR/.env" "$API_URL" "$DEFAULT_MODEL"
