...
export function ChatPanel() {
  const { currentSessionId, messages, addMessage, updateLastAgentMessage, loadMessages } = useStore();
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try {
        ...
        for await (const event of api.chat.sendStream(currentSessionId, userContent, attachmentIds)) {
        ...
        console.error('Stream failed, falling back to non-stream:', streamErr);
        try {
          const result = await api.chat.send(currentSessionId, userContent, attachmentIds);
          if (result.type === 'chat') {
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="chat-bubble-agent">
                  ...
                  <p className="whitespace-pre-wrap">
                    {msg.content}
                    {msg._streaming && streaming && (
                      <span className="inline-block w-1.5 h-4 bg-court-acc2/60 animate-pulse ml-0.5 align-middle" />
                    )}
                  </p>
                </div>
              </div>
